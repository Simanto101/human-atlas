...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  const coarse=typeof matchMedia!=='undefined'&&matchMedia('(pointer:coarse)').matches,mobileView=coarse||innerWidth<768;
  const maxDpr=Math.min(devicePixelRatio,mobileView?1.25:2);let dpr=mobileView?1:maxDpr,renderSamples=0,dprTime=0;
  const abort=new AbortController();
  let renderer:T.WebGLRenderer;
  try{renderer=new T.WebGLRenderer({antialias:!mobileView,alpha:false,powerPreference:'high-performance'});}catch{onError('This browser could not start the 3D viewer. Please try a browser with WebGL enabled.');return;}
   ...
   renderer.setPixelRatio(dpr);renderer.setClearColor(hex('--scene-clear')||0xecfeff);renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;el.appendChild(renderer.domElement);
  ...
  const scene=new T.Scene(),camera=new T.PerspectiveCamera(34,1,.002,320),controls=new OrbitControls(camera,renderer.domElement);
   camera.position.set(1.4,1.05,3.6);controls.target.set(0,.85,0);controls.enableDamping=true;controls.dampingFactor=.085;controls.minDistance=.006;controls.maxDistance=240;controls.maxPolarAngle=Math.PI;controls.minPolarAngle=0;controls.zoomToCursor=true;controls.enableZoom=true;controls.zoomSpeed=1.8;controls.addEventListener('change',()=>{dirty=true;});
  let env:T.Texture|null=null;
  if(!mobileView){const pmrem=new T.PMREMGenerator(renderer),envRoom=new RoomEnvironment();env=pmrem.fromScene(envRoom,.04).texture;scene.environment=env;envRoom.dispose();pmrem.dispose();}
   ...
   const fillLamp=new T.PointLight(0xd7f6fb,3.4,12,.9);fillLamp.position.set(0,3.4,2.4);scene.add(fillLamp);
   ...
   const applySceneTheme=()=>{
    ...
    fillLamp.intensity=dark?2.2:3.4;
    dirty=true;
   };
  applySceneTheme();
  ...
  const resize=()=>{layoutKey='';lastState=null;renderer.setPixelRatio(dpr);camera.aspect=el.clientWidth/el.clientHeight;camera.updateProjectionMatrix();renderer.setSize(el.clientWidth,el.clientHeight);fit(latest.current.view,amount);};const observer=new ResizeObserver(resize);observer.observe(el);
  ...
  const animate=()=>{
   ...
   if(isolateKey!==lastIsolate||(s.isolate&&moving)){
    if(s.isolate){const box=new T.Box3();atlas.parts.forEach((p,i)=>{if(s.selected.includes(p.id))box.union(bounds[i].clone().translate(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])));});
      if(!box.isEmpty()){const center=box.getCenter(new T.Vector3()),size=box.getSize(new T.Vector3());const w=el.clientWidth,h=el.clientHeight,mobile=w<768,landscape=w>h&&h<=600;let left=20,right=w-20,top=mobile?250:110,bottom=h-170;if(s.inspectorOpen){if(landscape){right=w-335;top=100;bottom=h-125;}else if(mobile){const sheet=document.querySelector('.detail-sheet')?.getBoundingClientRect(),header=document.querySelector('.identity')?.getBoundingClientRect();top=(header?.bottom??94)+16;bottom=(sheet?.top??h*.58-139)-16;}else{right=w-370;left=w>1100?285:25;}}const availableWidth=Math.max(150,right-left),availableHeight=Math.max(40,bottom-top);camera.setViewOffset(w,h,w/2-(left+right)/2,h/2-(top+bottom)/2,w,h);const distance=Math.max(.02,Math.max(size.y*h/availableHeight,size.x*w/availableWidth/camera.aspect,size.z)/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*1.35);controls.minDistance=.006;controls.maxDistance=240;controls.target.copy(center);camera.position.copy(center).add(new T.Vector3(.2,.1,1).normalize().multiplyScalar(distance));controls.update();dirty=true;}
    }else if(lastIsolate){camera.clearViewOffset();fit(s.view,amount);}
    lastIsolate=isolateKey;
   }
   controls.enableRotate=amount<.8;controls.mouseButtons.LEFT=amount<.8?T.MOUSE.ROTATE:T.MOUSE.PAN;controls.touches.ONE=amount<.8?T.TOUCH.ROTATE:T.TOUCH.PAN;platform.visible=ring.visible=innerRing.visible=amount<.5&&!s.isolate;markers.visible=amount>.75;controls.autoRotate=s.rotate&&!s.isolate&&amount<.4;controls.autoRotateSpeed=.65;controls.update();if(controls.autoRotate)dirty=true;
   if(dirty){renderer.render(scene,camera);renderSamples++;dprTime+=dt;if(renderSamples>=40){const avg=dprTime/renderSamples;renderSamples=0;dprTime=0;if(avg>.03&&dpr>.75){dpr=Math.max(.75,dpr-.25);renderer.setPixelRatio(dpr);}else if(avg<.018&&dpr<maxDpr){dpr=Math.min(maxDpr,dpr+.25);renderer.setPixelRatio(dpr);}}targets=[];if(amount>.45){const hasSolid=atlas.parts.some((p,i)=>!isBodySurface(p)&&data[i*4+3]>.5);atlas.parts.forEach((p,i)=>{if(data[i*4+3]<.5||(hasSolid&&isBodySurface(p)))return;let left=Infinity,right=-Infinity,top=Infinity,bottom=-Infinity;for(let corner=0;corner<8;corner++){projected.set(p.bounds[(corner&1)?1:0][0]+data[i*4],p.bounds[(corner&2)?1:0][1]+data[i*4+1],p.bounds[(corner&4)?1:0][2]+data[i*4+2]).project(camera);const x=(projected.x+1)*el.clientWidth/2,y=(1-projected.y)*el.clientHeight/2;left=Math.min(left,x);right=Math.max(right,x);top=Math.min(top,y);bottom=Math.max(bottom,y);}projected.copy(centers[i]).add(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])).project(camera);if(projected.z< -1||projected.z>1)return;targets.push({index:i,x:(projected.x+1)*el.clientWidth/2,y:(1-projected.y)*el.clientHeight/2,left,right,top,bottom});});}dirty=false;}
